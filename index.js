...
function createRock(x) {
  ...
  function moveRock() {
     ...
     */
       ...
       rock.remove(); // document.removeChild(rock);
     }
...
function endGame() {
  ...
  let rockCollection = document.getElementsByClassName('rock');
  for (let i=0; i<rockCollection.length; i++){
    rockCollection[i].remove();
  }
  document.removeEventListener('keydown', moveDodger) // to pass the current test
  window.removeEventListener('keydown', moveDodger) // to actually stop the listener
  alert("YOU LOSE!");
}
...
function moveDodgerLeft() {
   ...
   */
    ...
    if (dodgerLeftEdge > 0) {
			dodgerLeftEdge = positionToInteger(DODGER.style.left) - 4;
...
function moveDodgerRight() {
   ...
   */
		...
		var dodgerLeftEdge = positionToInteger(DODGER.style.left) + 4;

    if (dodgerLeftEdge < GAME_WIDTH-36) {
			dodgerLeftEdge = positionToInteger(DODGER.style.left) + 4;
			DODGER.style.left =  dodgerLeftEdge + "px";
