...
function checkCollision(rock) {
  ...
  if (top > GAME_HEIGHT - DODGER_HEIGHT - ROCK_HEIGHT) {
    ...
    return rockLeftEdge <= dodgerRightEdge && rockRightEdge >= dodgerLeftEdge
  }
}
...
function createRock(x) {
  ...
  function moveRock() {
    ...
    var rockBottom = positionToInteger(rock.style.top) + ROCK_HEIGHT

    if (rockBottom > GAME_HEIGHT) {
      return GAME.removeChild(rock)
    }
...
function moveDodger(e) {
  ...
  actions = {
    [LEFT_ARROW]: moveDodgerLeft,
    [RIGHT_ARROW]: moveDodgerRight
  }
...
function moveDodgerRight() {
  ...
  var left = positionToInteger(DODGER.style.left)
  var target = Math.min(left + 4, GAME_WIDTH - DODGER_WIDTH)
