...
function checkCollision(rock) {
  ...
  if (top > 360) {
    ...
    // FIXME: The DODGER is 40 pixels wide -- how do we get the right edge? Y
    const dodgerRightEdge = dodgerLeftEdge+40;

    const rockLeftEdge = positionToInteger(rock.style.left);

    // FIXME: The rock is 20 pixels wide -- how do we get the right edge? Y
    const rockRightEdge = rockLeftEdge+20;

    if((dodgerLeftEdge<=rockLeftEdge && dodgerRightEdge>=rockLeftEdge) || (dodgerLeftEdge<=rockRightEdge && dodgerRightEdge>=rockRightEdge)){
...
function endGame() {
  ...
  var ROCKSS = document.getElementsByClassName('rock');
  for (var i = ROCKSS.length; i--;) {
    ROCKSS[i].remove();
    }
